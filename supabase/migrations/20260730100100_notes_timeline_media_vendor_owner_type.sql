-- Vendors migration 2 of 5: widen the shared notes/timeline_activities/
-- media_assets CHECK constraints to accept owner_type = 'vendor', and widen
-- timeline_activities' type constraint to add the 5 new Vendor activity
-- types. Same one-migration-at-a-time widening discipline already
-- established for every previous module (most recently Inventory's
-- 20260729100200 migration).
--
-- media_assets is widened here (unlike the first draft of this migration)
-- because Vendors must support file/media attachments from the outset —
-- contracts, invoices, insurance documents, W-9s, tax documents,
-- certificates, catalogs, pricing sheets, logos, reference images — all via
-- the existing polymorphic Media Assets system. No separate Vendor
-- documents table is introduced.
--
...
alter table public.notes add constraint notes_owner_type_check
  check (owner_type in ('lead', 'client', 'event', 'contract', 'invoice', 'payment', 'expense', 'document', 'document_folder', 'inventory_item', 'vendor'));
...
alter table public.media_assets add constraint media_assets_owner_type_check
  check (owner_type in ('lead', 'client', 'event', 'document', 'inventory_item', 'vendor'));
