...
| favorite_colors / favorite_flowers / favorite_music / favorite_food / favorite_drinks / favorite_restaurants / preferred_style / disliked_elements | text | nullable couple-info preferences (`favorite_restaurants` added by `20260728100300_clients_favorite_restaurants.sql`, Clients Core-integration foundation phase) |
...
#### Clients Core-integration foundation phase — **additive, no `clients` table change beyond `favorite_restaurants`** (`supabase/migrations/20260728100300_clients_favorite_restaurants.sql`)

An additive-only extension of the existing Clients module (not a new module — see `src/core/README.md`'s Core domain for the shared services this integrates). Auditing the request against the already-shipped `clients` table found Address, Emergency Contact, Relationship Status, Communication Preferences, Preferences, and Important Dates already present (see the column table above); `favorite_restaurants` was the one genuinely missing sibling of `favorite_colors`/`favorite_flowers`/etc.

- **Notes, Timeline** — already fully wired via `owner_type = 'client'` (see the `notes`/`timeline_activities` sections above); no change needed.
- **Attachments** — already live via `MediaAsset` (`owner_type = 'client'` is in `LIVE_MEDIA_ASSET_OWNER_TYPES`, `src/lib/media/ownerTypes.ts`); no change needed.
- **Search registration** — already registered (`"client"` in `core/search/defaultRegistrations.ts`, Core Search phase); no change needed.
- **Tags (Core)** — the generic `core/tags` service already works for any `EntityType` including `"client"` via `getCoreTagsService().getTagsForOwner(workspaceId, "client", clientId)`; no Client-specific wrapper added, to avoid duplicating the pre-existing, unrelated `clients.tags: text[]` free-text feature.
- **Audit Log (Core)** — newly wired into `updateClientStatus`, `updateClientTags`, `setClientVipStatus`, `updateClientContactPreference`, `archiveClient`, and `restoreClient` (both `mockClientsRepository` and `supabaseClientsRepository`) — the six mutations that already get their own distinct Timeline activity type. Each records a `getCoreAuditLogService().recordAuditEvent(...)` call with the same `action` name as its Timeline `type`, and a field-level `before`/`after` diff. **Persists to Core Audit Log's in-memory mock store regardless of BloomOS's active data mode**, since Core Audit Log has no Supabase table yet (deferred in the Core foundation phase until a consumer needs real persistence) — entries do not survive a server restart in `supabase` data mode until that follow-up ships.
- **Notifications (Core)** — deliberately NOT auto-triggered from any Client mutation this phase. `getCoreNotificationsService()` works generically, but deciding a recipient for a Client-triggered notification needs the Team/permissions module (not itself in this phase's integration list), so it's a policy decision left for a future phase rather than a foundation-layer default.
- **Extension points** (Proposal History, Event History, Payment Summary, Document Summary, Communication History, AI Profile Summary) — `types/clientExtensions.ts` + `lib/data/clients/extensions.ts`'s `getClientExtensionSummary(clientId)`, exposed on `ClientsRepository`. Every field is a real, typed shape returning empty/null today — each is inherently a *derived* view of Leads/Events/Finance/Documents/Bloom AI, none of which this phase integrates. Not stored on `clients` (storing a snapshot would go stale immediately with nothing to populate it from).
