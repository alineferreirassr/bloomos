...
Fifth business module migrated to Supabase — same repository pattern as Leads/Clients/Events/Contracts (`lib/data/finance/`), bundling Invoices, Payments, Expenses, and their Notes/Timeline into one repository pair (a successful Payment mutation must atomically recompute its linked Invoice, so Invoices and Payments can't live in separate repository files without re-deriving that atomicity twice). Documents remains entirely mock-only; see "Supabase-specific notes" below.
...
- **The remaining mock-only module (Documents) still reads the mock `clients`/`events`/`contracts`/`invoices`/`payments`/`expenses` stores directly, not the live tables.** Documents (`validateDocumentOwnerAndReferences`, `attachDocumentToClient`/`attachDocumentToEvent`/`attachDocumentToContractExhibit`/`attachDocumentToInvoice`/`attachDocumentToPayment`/`attachDocumentToExpense`) cross-references those records via `readClients()`/`readEvents()`/`readContracts()`/`readInvoices()`/`readPayments()`/`readExpenses()` against the in-memory mock stores, unconditionally, regardless of `NEXT_PUBLIC_DATA_MODE`. In `supabase` mode this means Documents sees the mock stores' seeded records while the Clients/Events/Contracts/Finance modules themselves (list/detail/dashboard metrics) show live Supabase data — the two can disagree until Documents gets its own Supabase migration. This is the same shape of caveat the Leads migration created for `convertLeadToClient`, then Clients/Events for Contracts/Finance/Documents, then Contracts for Finance/Documents, now narrowed to Documents alone following the Finance migration. As with the Contracts migration, every Finance-domain function that reads Client/Event/Contract/Invoice/Payment/Expense data to compute a summary (`getContractFinanceSummary`, `getEventFinancialStatus`, `getFinanceDashboardData`, `getDashboardMetrics`'s Finance metrics) was updated to call the repository-routed `getContracts()`/`getEvents()`/`getInvoices()`/`getPayments()`/`getExpenses()` instead of reading any mock store directly — none of them silently return stale/zeroed totals for real Supabase-backed records.
- Row-Level Security (RLS) is **live** for `profiles`/`workspaces`/`workspace_members` (the Supabase Foundation), `leads`/`notes`/`timeline_activities`, `clients`, `events`/`checklist_items`/`event_schedule_items`, `media_assets`, `contracts`/`contract_templates`/`contract_exhibits`, and `invoices`/`payments`/`expenses` — a real Supabase project is connected (see `docs/integrations.md`). For every other table in this document (Documents, and beyond), RLS remains design-only — no migration exists for them yet. See `docs/permissions.md`.
