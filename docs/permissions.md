...
Access control model for BloomOS. A real Supabase project has been connected since the Supabase Foundation phase, and every migration referenced below has been applied to it — nothing in this document describes a design that's merely planned unless its own section says so explicitly (Team/Client Knowledge Base, Notification Center, and Automation Center remain genuinely unbuilt; see their own sections). Leads/Clients/Events/Contracts/Finance/Documents, the Team foundation (`roles`/`permissions`/`role_permissions`/`workspace_invitations`, plus the upgraded `workspace_members` policies), Client Accounts + Invitations, Client Portal MVP's additive client-facing policies, and Inventory/Vendors/Purchases/Services (active-membership-only, see below) all have live RLS today.
...
## Supabase Foundation RLS (live)

`supabase/migrations/20260715150600_rls_enablement.sql` enables RLS and defines every policy below on `profiles`, `workspaces`, and `workspace_members`. Applied to the live, connected Supabase project. No policy anywhere in this migration uses a bare `using (true)` — every rule is scoped to the requesting user's own row or their actual, active Workspace membership, and every policy is scoped `to authenticated` (so an anonymous/unauthenticated request, where `auth.uid()` is `null`, is rejected by every check below, not just implicitly).
...
## Commercial Pipeline (Booking Workflow, internal, live)

A new internal Sidebar group — **Pipeline** (`/pipeline/commercial`) — reuses the existing Lead/Event permission catalog unchanged; no new permission was introduced for this phase. Route access: `/pipeline/commercial` requires `leads.view` (`core/permissions/routeAccess.ts`), the same requirement as `/leads` itself, since the board is a Kanban view over the same Lead data. Within the page: viewing the board and every card needs nothing beyond that route-level `leads.view`; every stage-changing Quick Action (Assign Team, Schedule Consultation, Send Proposal, Move to Waiting Decision, Mark Lost) and drag-and-drop column move requires `leads.update`; Archive requires `leads.archive` (independent of `leads.update`, matching how Archive already works on the Leads list); Book Lead — and dropping a card into the "Booked" column — requires both `leads.update` **and** `events.create`, since `bookLead()` performs both a Lead status change and an Event creation. `modules/pipeline/quickActions.ts`'s `buildQuickActions()` is the single place these checks are applied, shared identically by the desktop board and the mobile single-stage view — an action neither view shows is one neither surface can trigger, not just a hidden button.

## Inventory, Vendors, Purchases, Services (live, active-membership-only)

These four modules — built after the Team Portal MVP, in the "Operational depth" phase (see `ROADMAP.md`) — have **no granular permission of their own**. `core/permissions/routeAccess.ts`'s `ROUTE_ACCESS_MAP` has no entry for `/inventory`, `/vendors`, `/purchases`, or `/services`, so `canAccessRoute()`/`getVisibleNavigationModules()` treat all four the same way an unlisted route is always treated — visible and reachable to any authenticated user with an `active` Workspace membership, regardless of role. There is no create/update/archive-level UI gating inside any of the four either (unlike Leads/Clients/Events/Contracts/Finance/Documents, which check `useMemberSession().can()` per action). RLS still enforces Workspace isolation on every table underneath them (`is_workspace_member(workspace_id)`, the same baseline every table gets) — a Staff member simply has the same read/write reach as an Owner within their own Workspace for these four modules today. This is a real, load-bearing gap, not an oversight to paper over: closing it means picking the actual permission names (following the existing `<module>.view`/`.create`/`.update`/`.archive` convention) and default role grants, a genuine product decision reserved for a future checkpoint rather than invented here.
...
- Granular permissions for Inventory, Vendors, Purchases, and Services — see "Inventory, Vendors, Purchases, Services (live, active-membership-only)" above; all four are reachable and fully editable by any active Workspace member today, with no per-role or per-action gate
