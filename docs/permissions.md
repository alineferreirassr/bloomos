...
| `payments` | `payments_select_client_account` | `is_client_account_holder_in_workspace(workspace_id, client_id)` — a client reaches a Payment only via its own Invoice's `client_id`, never a broader query. |
