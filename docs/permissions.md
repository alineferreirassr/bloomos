...
- **Client Portal landing page** (`/client-access`) — welcome message, Client name, account status, four "Coming soon" placeholder cards (Events/Contracts/Invoices/Documents), Sign Out. Exists only to verify routing/auth-separation/account-state behavior — the full Client Portal (real business data, real RLS scoping via `is_client_account_holder()`) remains separately-scoped future work.

**Explicitly not built this phase**: the full Client Portal (real Events/Contracts/Invoices/Documents views or their RLS), Team Portal persona invitations (below, still architecture-only), a production email provider, and `documents`/`document_folders` `visibility`-aware filtering (still Workspace-isolation-only — see "Documents visibility" below).
...
- **`touch_client_account_last_access()`** and **`get_current_client_account_context()`** — both narrowly-scoped `security definer` functions that only ever read/write the caller's own row, identified purely by `auth.uid()`, never a caller-supplied id. The latter is the sole path a Client Portal caller has to read even their own Client's display name this phase, since `clients` RLS remains Workspace-isolation-only and grants a client caller no other path to that table at all.
...
`documents.visibility`/`document_folders.visibility` (`internal`, `client`, `team`, `client_and_team`, `restricted` — `core/enums/documentVisibility.ts`) describe *intended* audience today, nothing more — no authentication or access-control code reads these values yet to filter what a caller can see, and every Document is reachable through the data layer by any Workspace member regardless of its `visibility`. This is deliberate groundwork, not a gap being papered over: the security architecture is designed now so the enforcement layer has a real field to key off once a Client Portal / Team Portal role exists, rather than retrofitting visibility onto Documents after the fact.
...
- The full Client Portal — real Events/Contracts/Invoices/Documents views for a client, and the RLS to scope them via `is_client_account_holder()`. The account/invitation foundation (`client_accounts`/`client_invitations`) is live (see "Client accounts and invitations" above); only the business-data-facing UI and its RLS remain future scope.
- Enforcing `documents`/`document_folders` `visibility` at the data-layer or RLS level beyond plain Workspace isolation (the future Client Portal's scoped filtering by `visibility` is not built yet)
