...
The authentication, account-linking, and invitation foundation for **external** Amoré Bloom clients — the second Phase 2 module, on `feature/client-access`. Deliberately separate from Team membership above: a client account is never a `workspace_members` row, an internal team member and a Client Portal user are distinguished by which linking table has a row for them (never by an email-domain assumption), and a client never gains internal Workspace access. Canonical model: Auth User → `client_accounts` row → exactly one `clients` record → (future phase) permitted Events/Contracts/Invoices/Documents. Explicitly out of scope this phase: the full Client Portal UI (real Events/Contracts/Invoices/Documents views), Team Portal persona invitations, both Knowledge Bases, the Notification Center, and the Automation Center.
...
**Client Portal landing page** (`/client-access`, `ClientAccessLandingView.tsx`) — welcome message, Client name, account status, four placeholder cards (Events/Contracts/Invoices/Documents, all "Coming soon" — no real data), and Sign Out. Exists only to verify client-only routing, account context, auth separation, and active/suspended/revoked behavior — not the full Client Portal, which remains separately-scoped future work. Touches `last_access_at` once per visit via the narrowly-scoped `touch_client_account_last_access()` RPC.
