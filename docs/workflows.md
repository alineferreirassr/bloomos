...
**Client Portal shell nav** (`components/layout/ClientPortalShell.tsx`, extended this phase) — desktop horizontal nav (Overview/My Events/My Contracts/My Invoices/My Documents/Account) with active-route highlighting via `usePathname()` and a longest-prefix match, a mobile hamburger menu with the same items plus Sign Out, and "Client Portal" branding — still never `AppShell`, never an internal Sidebar or Team navigation link.

**Blocked states, audited and completed** — `ClientAccountSessionSnapshot` distinguishes `unauthenticated` (redirects to `/sign-in`), `no-account`, `blocked` (`suspended`/`revoked`), and `error` — the last one added after an audit pass found `resolveClientAccountSessionSnapshot()` had no error handling around its repository call: a thrown error (network/DB failure) became an unhandled promise rejection, stranding the layout on its loading `Skeleton` forever with no safe state to render and no way out. Every blocked variant now reuses `AccessBlockedPage` with a `brandSuffix="Client Portal"` qualifier (a small addition to that shared component, optional and unused by its internal Team Portal caller) — so a blocked Client Portal page always reads "Amoré Bloom" plus "Client Portal," never the bare internal brand alone, matching every other Client Portal surface.
...
**Architecture audit and fixes (post-launch pass)** — a follow-up audit confirmed the pages above conform to the intended architecture, and found two real gaps, both fixed without touching any route, repository function, or working page: `resolveClientAccountSessionSnapshot()` now wraps its repository call so a thrown error resolves to a distinct `{ kind: "error" }` snapshot instead of an unhandled rejection that stranded the layout on its loading skeleton forever (see "Blocked states, audited and completed" below); every Client Portal blocked state now carries an explicit "Client Portal" brand qualifier via `AccessBlockedPage`'s new optional `brandSuffix` prop, so it never reads as the bare internal brand. The shell's nav labels were also renamed to their canonical form ("My Events"/"My Contracts"/"My Invoices"/"My Documents") to match this document's own wording above.

## Client Portal administration (internal, live)

A new first-class internal Sidebar/MobileNav group — **Client Portal** — distinct from **Clients** (the CRM record) and never merged into it: Clients is the internal relationship record; Client Portal administration is external account/access management for that same Client. Two new pages, both Workspace-wide admin views mirroring the existing Team Members/Invitations pattern, reusing `ClientAccessRepository` unchanged (no new repository logic, no new permissions, no schema/RLS change):

- **`/client-portal/accounts`** (`ClientAccountsAdminView.tsx`) — every `ClientAccount` across the Workspace (`getClientAccounts()`), joined with `getClients()` for the linked-client name and a link to that Client's own detail page. Search (email or client name), status filter, suspend/reactivate/revoke actions gated by `clients.portal_suspend`/`clients.portal_manage`, last-access column.
- **`/client-portal/invitations`** (`ClientInvitationsAdminView.tsx`) — every `ClientInvitation` across the Workspace, same search/filter/client-link pattern, resend/revoke gated by `clients.portal_invite`, expiration display. Creating a *new* invitation still only happens from Client Detail's existing `ClientAccessSection` (unchanged) — this page manages already-created invitations only.

Both pages are route-guarded by `/client-portal`'s entry in `core/permissions/routeAccess.ts` (`clients.portal_view`, the same "broad view for every role" precedent as `/team`), added to `PROTECTED_ROUTE_PREFIXES`, and rendered inside the existing `RouteGuard`/internal `AppShell` — this is purely an internal admin surface, never reachable by a Client Portal user.
