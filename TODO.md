...
- [ ] Build Finance module, then wire its card into the Dashboard — foundation done on `feature/finance`: canonical `Invoice`/`Payment`/`Expense` types, integer minor-unit money model (`lib/money.ts`), three independent state machines (`core/workflows/invoiceWorkflow.ts`/`paymentWorkflow.ts`/`expenseWorkflow.ts`), full CRUD + lifecycle actions in `lib/data/index.ts` (including Payment-to-Invoice application and the refund model), event/workspace financial summaries, derived Event financial status, shared Notes/Timeline reuse, realistic seed data across every status, Dashboard metrics wired. Still pending: Finance UI (invoice/payment/expense list, detail, create/edit), payment-provider integration, Documents module (file upload for receipts/confirmations)
