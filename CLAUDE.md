...
- **No fake or complex auth.** Authentication integrates with Supabase Auth when credentials exist. Do not build parallel/fictional auth systems.
- **Supabase stays disconnected until real credentials exist.** Do not scaffold a live Supabase connection, run migrations against a real project, or fabricate a project URL/anon key. Schema and RLS design can and should be documented and prepared in advance.
