...
describe("archiveWorkflow / unarchiveWorkflow", () => {
  ...
  it("archives, then unarchives, a Workflow the member owns", async () => {
    vi.mocked(resolveMemberSessionSnapshot).mockResolvedValue(activeSession);
    const workflow = await createTestWorkflow();
    ...
    const unarchived = await unarchiveWorkflow(workflow.id);
    expect(unarchived.success).toBe(true);
    if (unarchived.success) expect(unarchived.data.status).toBe("draft");
  });
});
