...
import { getMediaKitForWorkspace, reorderMediaKitGalleryItemsForWorkspace } from "@/lib/data/mediaKit";
...
export async function reorderMediaKitGalleryItemsAction(portfolioItemId: string | null, orderedItemIds: string[]): Promise<DataResult<MediaKitGalleryItem[]>> {
  ...
  return reorderMediaKitGalleryItemsForWorkspace(session.workspace.id, mediaKit.id, portfolioItemId, orderedItemIds);
}
