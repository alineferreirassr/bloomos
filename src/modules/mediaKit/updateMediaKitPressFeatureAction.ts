...
import { updateMediaKitPressFeatureForWorkspace } from "@/lib/data/mediaKit";
...
export async function updateMediaKitPressFeatureAction(pressFeatureId: string, input: MediaKitPressFeatureInput): Promise<DataResult<MediaKitPressFeature>> {
  ...
  return updateMediaKitPressFeatureForWorkspace(session.workspace.id, pressFeatureId, normalized);
}
