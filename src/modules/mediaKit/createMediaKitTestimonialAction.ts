...
import { createMediaKitTestimonialForWorkspace, getMediaKitForWorkspace } from "@/lib/data/mediaKit";
...
/** Never auto-approved on creation — `is_approved` always starts false, the schema default; approval is only ever the dedicated setMediaKitTestimonialApprovedAction. */
export async function createMediaKitTestimonialAction(input: MediaKitTestimonialInput): Promise<DataResult<MediaKitTestimonial>> {
  ...
  return createMediaKitTestimonialForWorkspace(session.workspace.id, mediaKit.id, normalized);
}
