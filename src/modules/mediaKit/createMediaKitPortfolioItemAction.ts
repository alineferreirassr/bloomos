...
import { createMediaKitPortfolioItemForWorkspace, getMediaKitForWorkspace } from "@/lib/data/mediaKit";
...
/** Editorial curation only — a portfolio piece may optionally link an `event_id`, but nothing here is ever auto-derived from it. */
export async function createMediaKitPortfolioItemAction(input: MediaKitPortfolioItemInput): Promise<DataResult<MediaKitPortfolioItem>> {
  ...
  return createMediaKitPortfolioItemForWorkspace(session.workspace.id, mediaKit.id, normalized);
}
