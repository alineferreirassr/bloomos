...
import { getMediaKitForWorkspace, reorderMediaKitPartnersForWorkspace } from "@/lib/data/mediaKit";
...
export async function reorderMediaKitPartnersAction(orderedPartnerIds: string[]): Promise<DataResult<MediaKitPartner[]>> {
  ...
  return reorderMediaKitPartnersForWorkspace(session.workspace.id, mediaKit.id, orderedPartnerIds);
}
