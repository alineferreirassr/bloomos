  ...
  return (
    <Card>
      ...
      {message ? (
        <div
          role="alert"
          className={`mt-3 rounded-lg border px-3 py-2 text-xs ${message.tone === "error" ? "border-danger/40 bg-danger/10 text-danger" : "border-success/40 bg-success/10 text-success"}`}
        >
          {message.text}
        </div>
      ) : null}
