...
async function loadEventServiceAttachments(eventServiceId: string): Promise<LoadState> {
  try {
    const assets = await getMediaAssetsByOwner("event_service", eventServiceId, { includeArchived: true });
...
/**
 * Images, files, reference material, invoices — all Media Assets scoped
 * directly to this EventService (`owner_type: "event_service"`, widened
 * onto `media_assets` specifically for this Workspace — see
 * `20260807100000_media_event_service_owner_type.sql`). Modeled closely on
 * `VendorDocumentsSection` (the closest existing reference for this exact
 * upload/list/open/archive/restore shape over raw Media Assets, no
 * Documents-table metadata) rather than duplicated ad hoc — the one
 * difference is the read-only gate below, since unlike a Vendor an
 * EventService does reach terminal states.
 */
export function EventServiceAttachmentsSection({ eventServiceId, status }: { eventServiceId: string; status: EventServiceStatus }) {
  ...
  useEffect(() => {
    let cancelled = false;
    loadEventServiceAttachments(eventServiceId).then((next) => {
  ...
  const refetch = () => {
    loadEventServiceAttachments(eventServiceId).then(setState);
  };
  ...
  const handleFileSelected = async (file: File | undefined) => {
    ...
    setIsUploading(true);
    const result = await uploadMediaAsset({ ownerType: "event_service", ownerId: eventServiceId, file, originalFilename: file.name });
  ...
  if (state.status === "error") {
    return <ErrorState message="Could not load this assignment's attachments." onRetry={retry} />;
  }
    ...
    <div className="space-y-4">
      {!readOnly ? (
        <div className="flex items-center justify-between">
          <label htmlFor="event_service_attachment_upload" className="text-sm font-medium text-text-muted">
            Upload a file
          </label>
          ...
          <input
            ref={fileInputRef}
            id="event_service_attachment_upload"
            type="file"
            className="sr-only"
      ...
      {assets.length === 0 ? (
        <EmptyState title="No attachments yet" description={readOnly ? "No files were attached to this assignment." : "Upload the first file for this assignment."} />
      ) : (
