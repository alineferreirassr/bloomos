 ...
 */
export function LittleReminderCard({ reminder }: { reminder: LittleReminderData | null }) {
  return (
    <LuxuryCard>
      <div className="flex items-center gap-2">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-luxury-md bg-luxury-blush text-luxury-rose">
          <LuxuryBellIcon className="h-4 w-4" />
        </span>
        <p className="text-luxury-small font-semibold text-luxury-rose">Little Reminder ♡</p>
      </div>
      {reminder ? (
        <>
          <p className="mt-3 text-luxury-body font-semibold text-luxury-text">{reminder.title}</p>
          <p className="mt-0.5 text-luxury-small text-luxury-text-muted">{reminder.body}</p>
        </>
      ) : (
        <p className="mt-3 text-luxury-small text-luxury-text-muted">Small steps still move you forward.</p>
      )}
    </LuxuryCard>
  );
}
