...
/**
 * Centralized DocumentFolder tree logic — every rule about nesting, cycles,
 * and cross-Workspace/cross-owner moves lives here so the data layer (and
 * any future folder-tree UI) never reimplements tree-walking by hand.
 */
 ...
 * folder always stays within the single owner it was created for.
 */
...
export function getFolderPath(folderId: string, allFolders: DocumentFolder[]): DocumentFolder[] {
  const byId = new Map(allFolders.map((folder) => [folder.id, folder]));
...
export function getFolderChildren(parentFolderId: string | null, allFolders: DocumentFolder[]): DocumentFolder[] {
...
export function getFolderDescendants(folderId: string, allFolders: DocumentFolder[]): DocumentFolder[] {
  const descendants: DocumentFolder[] = [];
...
export function sortFolders(folders: DocumentFolder[]): DocumentFolder[] {
