...
describe("mockFinanceRepository.createExpense / updateExpense — Event/Contract must belong to Client", () => {
  ...
  it("updateExpense is blocked once the expense is terminal", async () => {
    const created = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, crypto.randomUUID());
    expect(created.success).toBe(true);
    if (!created.success) return;
...
describe("mockFinanceRepository.createExpense — request idempotency (Finance F2.1C-F-E-D-B2)", () => {
  it("rejects a missing (empty-string) expenseId", async () => {
    const result = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, "");
    expect(result.success).toBe(false);
  });

  it("a same-key retry with the same payload replays the original Expense — no second Expense", async () => {
    const id = crypto.randomUUID();
    const first = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, id);
    expect(first.success).toBe(true);
    const second = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, id);
    expect(second.success).toBe(true);
    if (!first.success || !second.success) return;

    expect(second.data.id).toBe(first.data.id);

    const allExpenses = await mockFinanceRepository.getExpenses();
    expect(allExpenses.filter((e) => e.id === first.data.id)).toHaveLength(1);
  });

  it("a same-key retry with a DIFFERENT description is rejected as a conflict, not replayed", async () => {
    const id = crypto.randomUUID();
    const first = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, id);
    expect(first.success).toBe(true);

    const conflicting = await mockFinanceRepository.createExpense({ ...BASE_EXPENSE_INPUT, description: "A different expense" }, id);
    expect(conflicting.success).toBe(false);
    if (conflicting.success) return;
    expect(conflicting.error).toMatch(/idempotency key was already used/);
  });

  it("a same-key retry with a DIFFERENT amount_minor is rejected as a conflict", async () => {
    const id = crypto.randomUUID();
    const first = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, id);
    expect(first.success).toBe(true);

    const conflicting = await mockFinanceRepository.createExpense({ ...BASE_EXPENSE_INPUT, amount_minor: 999900 }, id);
    expect(conflicting.success).toBe(false);
  });

  it("a same-key retry with a DIFFERENT category is rejected as a conflict", async () => {
    const id = crypto.randomUUID();
    const first = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, id);
    expect(first.success).toBe(true);

    const conflicting = await mockFinanceRepository.createExpense({ ...BASE_EXPENSE_INPUT, category: "software" }, id);
    expect(conflicting.success).toBe(false);
  });

  it("a DIFFERENT key represents a distinct, intentional second identical Expense", async () => {
    const first = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, crypto.randomUUID());
    expect(first.success).toBe(true);
    const second = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, crypto.randomUUID());
    expect(second.success).toBe(true);
    if (!first.success || !second.success) return;
  ...
  it("a same-key collision against a historical Expense with no recorded snapshot fails safely as a conflict, never as a silent replay", async () => {
    // expense_1 is a seeded historical row (predates this mechanism) — its
    // id was never generated by createExpense, so it has no snapshot.
    const result = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, "expense_1");
    expect(result.success).toBe(false);
    if (result.success) return;
    expect(result.error).toMatch(/idempotency key was already used/);

    // Confirm no mutation occurred — the historical row is untouched.
    const historical = await mockFinanceRepository.getExpenseById("expense_1");
    expect(historical.description).not.toBe(BASE_EXPENSE_INPUT.description);
  });

  it("historical replay after a later edit: a stale retry of the original create payload still replays safely, and the edit is preserved (never overwritten)", async () => {
    const id = crypto.randomUUID();
    const created = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, id);
    expect(created.success).toBe(true);
    if (!created.success) return;

    const edited = await mockFinanceRepository.updateExpense(created.data.id, {
      ...BASE_EXPENSE_INPUT,
      description: "Re-described after creation",
    });
    expect(edited.success).toBe(true);
    if (!edited.success) return;
    expect(edited.data.description).toBe("Re-described after creation");

    // The ORIGINAL stale request (same id, same ORIGINAL payload) finally
    // arrives after the edit above. It must still be recognized as a safe
    // replay of the original creation — never re-validated against the
    // now-different current description — and must return the CURRENT
    // (edited) Expense untouched, never reverting the Founder's edit.
    const staleRetry = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, id);
    expect(staleRetry.success).toBe(true);
    if (!staleRetry.success) return;
    expect(staleRetry.data.id).toBe(created.data.id);
    expect(staleRetry.data.description).toBe("Re-described after creation");

    const allExpenses = await mockFinanceRepository.getExpenses();
    expect(allExpenses.filter((e) => e.id === created.data.id)).toHaveLength(1);
  });

  it("duplicateExpense's resulting row has no recorded snapshot and therefore can never be silently 'replayed' by an unrelated createExpense request reusing its id", async () => {
    const created = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, crypto.randomUUID());
    expect(created.success).toBe(true);
    if (!created.success) return;

    const duplicate = await mockFinanceRepository.duplicateExpense(created.data.id);
    expect(duplicate.success).toBe(true);
    if (!duplicate.success) return;

    const collision = await mockFinanceRepository.createExpense(BASE_EXPENSE_INPUT, duplicate.data.id);
    expect(collision.success).toBe(false);
  });
});
...
describe("mockFinanceRepository Expense status lifecycle", () => {
  ...
  it("rejects markExpenseReimbursed when the expense isn't reimbursable", async () => {
    const created = await mockFinanceRepository.createExpense({ ...BASE_EXPENSE_INPUT, reimbursable: false }, crypto.randomUUID());
    expect(created.success).toBe(true);
    if (!created.success) return;
    const id = created.data.id;
