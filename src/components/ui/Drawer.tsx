 ...
 * Entrance uses the shared Luxury Motion System's `animate-drawer-in` (a
 * gentle slide-from-edge, distinct from Modal's own `animate-modal-in`
 * scale-in) — both collapse automatically under `prefers-reduced-motion`
 * via the sitewide rule in globals.css.
 */
export function Drawer({ open, onClose, title, children }: DrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-[var(--z-index-modal)] flex justify-end">
      <button type="button" aria-label="Close dialog" onClick={onClose} className="animate-fade-in absolute inset-0 bg-neutral-800/50" />
      <div
        ref={panelRef}
        role="dialog"
        aria-modal="true"
        aria-label={title}
        tabIndex={-1}
        className="bloom-elevation-modal animate-drawer-in relative flex h-full w-full max-w-[420px] flex-col gap-3.5 border-l border-border bg-surface p-4 focus:outline-none"
      >
