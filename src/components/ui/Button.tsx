...
const variantClasses: Record<ButtonVariant, string> = {
  primary: "bg-accent text-accent-foreground shadow-sm hover:opacity-90 active:opacity-95",
  secondary: "border border-border bg-surface text-text hover:bg-surface-tint active:bg-surface-tint",
  ghost: "text-accent hover:bg-accent-100 active:bg-accent-100",
};
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  return (
    <button
      ref={ref}
      className={`inline-flex items-center justify-center gap-1.5 rounded-md border border-transparent font-serif font-semibold transition-colors duration-150 disabled:pointer-events-none disabled:opacity-45 ${variantClasses[variant]} ${sizeClasses[size]} ${className}`}
      {...props}
    />
  );
});
